    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box box-danger">
                ...
                <form action="{{route('post-validatorEditUserPicTelkom',['id' => $user->id])}}" class="form-horizontal"
                      method="POST">
                    {{csrf_field()}}
                    <div class="box-body">
                                    ...
                                    @foreach($picTelkoms as $picTelkom)
                                        @if(old('picTelkom') == $picTelkom->id)
                                            <option title="" value="{{$picTelkom->id}}"
                                                    selected>{{$picTelkom->nama}}({{$picTelkom->unit}})</option>
                                        @else
                                            @if($user->pengguna->idPicTelkom == $picTelkom->id)
                                                <option title="" selected value="{{$picTelkom->id}}">{{$picTelkom->nama}}({{$picTelkom->unit}})</option>
                                            @else
                                                <option title="" value="{{$picTelkom->id}}">{{$picTelkom->nama}}({{$picTelkom->unit}})</option>
                                            @endif
                                        @endif
                                    @endforeach
                                </select>
                    ...
                    <div class="box-body">
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                ...
                                @else
                                    <input type="text" class="form-control" name="username" id="username"
                                           placeholder="Username" required autocomplete="off" value="{{$user->username}}">
                                @endif
                    ...
                    <div class="box-footer">
                        ...
                        <a href="{{route('get-validatorLihatUserPicTelkom')}}">
