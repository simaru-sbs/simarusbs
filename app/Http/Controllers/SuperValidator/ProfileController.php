...
class ProfileController extends Controller
{
    public function indexProfile()
    {
        ...
        return view('SuperValidator/editProfile', compact('user'));
    }
    ...
    public function editProfile(Request $request)
    {
        ...
        if ($request->password || $request->validatePassword) {
            if ($request->password != $request->validatePassword) {
                return redirect()->route('supervalidator-indexProfile')
                    ->with([
                        'status' => 'danger',
        ...
        if (auth('user')->user()->username != $request->username) {
            ...
            if ($checkUsername != 0) {
                return redirect()->route('supervalidator-indexProfile', ['id' => $request->id])
                    ->with([
                        'status' => 'danger',
