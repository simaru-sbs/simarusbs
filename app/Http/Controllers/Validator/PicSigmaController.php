...
class PicSigmaController extends Controller
{
    public function indexPic()
    {
        ...
        return view('Validator/Sigma/buatPicSigma', compact('units'));
    }

    public function indexUnit()
    {
        return view('Validator/Sigma/buatUnitSigma');
    }
    ...
    public function indexEditUnit($id)
    {
        ...
        if (!$unit) {
            return redirect()->route('get-validatorLihatUnitSigma')
                ->with([
                    'status' => 'danger',
                    'message' => 'Data unit SIGMA dengan ID tersebut tidak ditemukan.'
                ]);
        }
        return view('Validator/Sigma/editUnitSigma', compact('unit'));
    }
    ...
    public function indexEditPic($id)
    {
        ...
        return view('Validator/Sigma/editPicSigma', compact('units', 'pic'));
    }
    ...
    public function buatPic(Request $request)
    {
        ...
        if ($checkPic != 0) {
            return redirect()->route('get-validatorBuatPicSigma')
                ->with([
                    'status' => 'danger',
    ...
    public function buatUnit(Request $request)
    {
        ...
        if ($checkUnit != 0) {
            return redirect()->route('get-validatorBuatUnitSigma')
                ->with([
                    'status' => 'danger',
                    'message' => 'Unit Perusahaan ' . $request->namaUnit . ' sudah ada!'
                ])->withInput();
        }

        $unitSigma = UnitPerusahaan::create([
        ...
        return redirect()->route('get-validatorBuatUnitSigma')
            ->with([
                'status' => 'success',
                'message' => 'Unit SIGMA ' . $request->namaUnit . ' berhasil ditambahkan!'
            ]);
    }
    ...
    public function lihatPic()
    {
        ...
        return view('Validator/Sigma/lihatPicSigma', compact('picSigmas'));
    }
    ...
    public function lihatUnit()
    {
        ...
        return view('Validator/Sigma/lihatUnitSigma', compact('unitSigmas'));
    }
    ...
    public function editUnit(Request $request)
    {
        ...
        if ($checkUnit != 0) {
            return redirect()->route('get-validatorEditUnitSigma', ['id' => $request->id])
                ->with([
                    'status' => 'danger',
    ...
    public function editPic(Request $request)
    {
        ...
        if ($namaLama != $request->nama) {
            ...
            if ($checkPic != 0) {
                return redirect()->route('get-validatorEditPicSigma', ['id' => $request->id])
                    ->with([
                        'status' => 'danger',
