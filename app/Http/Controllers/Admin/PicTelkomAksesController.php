...
class PicTelkomAksesController extends Controller
{
    public function indexPic()
    {
        ...
        return view('Admin/TelkomAkses/buatPicTelkomAkses', compact('units'));
    }

    public function indexUnit()
    {
        return view('Admin/TelkomAkses/buatUnitTelkomAkses');
    }
    ...
    public function indexEditUnit($id)
    {
        ...
        if (!$unit) {
            return redirect()->route('get-lihatUnitTelkomAkses')
                ->with([
                    'status' => 'danger',
        ...
        return view('Admin/TelkomAkses/editUnitTelkomAkses', compact('unit'));
    }
    ...
    public function indexEditPic($id)
    {
        ...
        if (!$pic) {
            return redirect()->route('get-lihatPicTelkomAkses')
                ->with([
                    'status' => 'danger',
                    'message' => 'Data PIC Telkom Akses dengan ID tersebut tidak ditemukan.'
                ]);
        }
        return view('Admin/TelkomAkses/editPicTelkomAkses', compact('units', 'pic'));
    }
    ...
    public function buatPic(Request $request)
    {
        ...
        if ($checkPic != 0) {
            return redirect()->route('get-buatPicTelkomAkses')
                ->with([
                    'status' => 'danger',
    ...
    public function buatUnit(Request $request)
    {
        ...
        if ($checkUnit != 0) {
            return redirect()->route('get-buatUnitTelkomAkses')
                ->with([
                    'status' => 'danger',
        ...
        return redirect()->route('get-buatUnitTelkomAkses')
            ->with([
                'status' => 'success',
                'message' => 'Unit Telkom Akses ' .strtoupper($request->namaUnit). ' berhasil ditambahkan!'
            ]);
    }
    ...
    public function lihatPic()
    {
        ...
        return view('Admin/TelkomAkses/lihatPicTelkomAkses', compact('picTas'));
    }
    ...
    public function lihatUnit()
    {
        ...
        return view('Admin/TelkomAkses/lihatUnitTelkomAkses', compact('unitTas'));
    }
    ...
    public function editUnit(Request $request)
    {
        ...
        if ($checkUnit != 0) {
            return redirect()->route('get-editUnitTelkomAkses', ['id' => $request->id])
                ->with([
                    'status' => 'danger',
    ...
    public function editPic(Request $request)
    {
        ...
        if ($nikLama != $request->nik) {
            ...
            if ($checkPic != 0) {
                return redirect()->route('get-editPicTelkomAkses', ['id' => $request->id])
                    ->with([
                        'status' => 'danger',
