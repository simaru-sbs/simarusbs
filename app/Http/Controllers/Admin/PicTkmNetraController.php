...
class PicTkmNetraController extends Controller
{
    ...
    {
        return view('Admin/TkmNetra/buatPicTkmNetra');
    }
    ...
    public function indexEditPic($id)
    {
        ...
        if (!$pic) {
            return redirect()->route('get-lihatPicTkmNetra')
                ->with([
                    'status' => 'danger',
                    'message' => 'Data PIC TKM NETRA dengan ID tersebut tidak ditemukan.'
                ]);
        }
        return view('Admin/TkmNetra/editPicTkmNetra', compact('pic'));
    }
    ...
    public function buatPic(Request $request)
    {
        ...
        if ($checkPic != 0) {
            return redirect()->route('get-buatPicTkmNetra')
                ->with([
                    'status' => 'danger',
    ...
    public function lihatPic()
    {
        ...
        return view('Admin/TkmNetra/lihatPicTkmNetra', compact('picTkmNetra'));
    }
    ...
    public function editPic(Request $request)
    {
        ...
        if ($nikLama != $request->nik) {
            ...
            if ($checkPic != 0) {
                return redirect()->route('get-editPicTkmNetra', ['id' => $request->id])
                    ->with([
                        'status' => 'danger',
